...
class RecurringShoppingItemController {
  ...
  async previewMatches(req: AuthRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      if (!req.user) {
        res.status(401).json({ status: 'error', message: 'Unauthorized' });
        return;
      ...
      const householdId = req.params.id as string;
      const { triggerWords, category } = req.body as {
        triggerWords: string[];
        category?: ExpenseType;
      };
      const result = await recurringShoppingItemService.previewMatches(
        householdId,
        req.user.userId,
        triggerWords,
        category
